structure ServerSerialize :> SERVER_SERIALIZE =
struct
...
fun pair (t1: 'a t) (t2: 'b t) =
    let (* Type pair is printed: (type1,type2) *)
        val name = "(" ^ (#name t1) ^ "," ^ (#name t2) ^ ")"
        fun to_string (a,b) =
            let val a_s = (#to_string t1) a
                val a_sz = Int.toString (String.size a_s)
                val b_s = (#to_string t2) b
            in a_sz ^ ":" ^ a_s ^ b_s
            end
        fun from_string s =
            let val s' = Substring.full s
                val (a_sz,rest) =
                    Option.valOf (Int.scan StringCvt.DEC Substring.getc s')
                val rest = #2(Option.valOf (Substring.getc rest)) (* skip ":" *)
                val (a_s,b_s) = (Substring.slice(rest,0,SOME a_sz),
                                 Substring.slice(rest,a_sz,NONE))
...
fun option (t : 'a t) =
    let (* Option type is printed: O(type) *)
        val name = "O(" ^ (#name t) ^ ")"
        fun to_string a =
            case a of
                NONE => "0:N()"
              | SOME v =>
                let val v_s = (#to_string t) v
        ...
        fun from_string s =
            let val s' = Substring.full s
                ...
                val rest = #2(Option.valOf (Substring.getc rest)) (* skip "(" *)
            in if N_S = #"S"
               then SOME ((#from_string t) (Substring.string (Substring.slice(rest,0,SOME v_sz))))
               else NONE
            end
    in {name=name,
        to_string=to_string,
...
fun list (t : 'a t) =
    let (* List type is printed: L(type) *)
        val name = "L(" ^ (#name t) ^ ")"
        (* Format: [x1_sz:x1...xN_sz:xN] *)
        fun to_string xs =
            let fun to_string_x x =
                    let val v_x = (#to_string t) x
                    in Int.toString (String.size v_x) ^ ":" ^ v_x
                    end
                val xs' = List.map to_string_x xs
            in "[" ^ (String.concat xs') ^ "]"
            end
        fun from_string s =
            let fun read_x (rest,acc) =
                    if Substring.size rest = 1 (* "]" *)
                    then List.rev acc
                    else
                      let val (x_sz,rest) = Option.valOf (Int.scan StringCvt.DEC Substring.getc rest)
                          val rest = #2(Option.valOf (Substring.getc rest)) (* skip ":" *)
                          val (x_s,rest) = (Substring.slice(rest,0,SOME x_sz),Substring.slice(rest,x_sz,NONE))
                      in read_x (rest,((#from_string t) (Substring.string x_s)) :: acc)
                      end
...
fun triple (t1: 'a t) (t2: 'b t) (t3: 'c t) =
    let (* Type triple is printed (type1,type2,type3) *)
...
(* Pre defined types *)
val unit   = {name="U",to_string=fn () => "unit",
              from_string=(fn "unit" => () | _ => raise Option.Option)}
